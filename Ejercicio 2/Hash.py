...
class Hash:
    def __init__(self, dimension, primo=False):
        ...
        self.__numeroAleatorio = randint(1, self.__capacidad)
    ...
    def insertar(self, clave):
        pos = self.__pruebaPseudoRandom(clave)
    ...
    def longitud(self, clave):
        pos = self.__funcionHash(clave)
        cont = 0
        while self.__array[pos] != clave and cont < self.__capacidad:
            pos = self.__funcionHash(pos + self.__numeroAleatorio)
            cont += 1
        return cont
